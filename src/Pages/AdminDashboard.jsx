...
const AdminDashboard = () => {
  ...
  const fetchUsers = async () => {
    try {
      // ✅ Corrected: Added params for pagination
      const response = await axios.get('http://localhost:5000/api/admin/users', {
        headers: { Authorization: `Bearer ${token}` },
        params: { page: currentPage, per_page: perPage }
      });
      console.log('Users response:', response.data);
      // ✅ Corrected: Access the 'items' array from the response data
      setUsers(response.data.items || []);
      // ✅ Corrected: Get total pages from the API response
  ...
  const fetchDisputes = async () => {
    try {
      // ✅ Corrected: Added params for pagination (optional but good practice)
